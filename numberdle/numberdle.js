...
const legalLetters = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"];
...
function getTodaysWord(randomString = new Date().toDateString()) {
    ...
    for (let i = 0; i < numLetters; i++) {
        let digit = Math.floor(seededRandom(rSeed, 10)());
        if (digit === 11) {
            digit -= 1;
        }
        rWord[i] = digit;
...
function handleKey(key) {
    ...
    if (key === "←") {
        ...
        return;
    } else if (currentGuess.length < numLetters && legalLetters.includes(key)) {
        currentGuess += key;
    }
...
function copyResults() {
    if (gameState == "win" || gameState == "fail") {
        textToCopy = "";
        textToCopy += `Numberdle #${daysSinceStart()}\n`
        if (gameState == "win") {
...
function daysSinceStart() {
    ...
    const baseDate = new Date(2024, 6 - 1, 23);
